...
- **`describe-image /path/to/image [question]`** — "See" an image. Uses OpenAI gpt-4o-mini vision (~$0.0001-0.0005). Use this for ANY question about an image ("qué hay aquí", OCR, count people, etc.) when the model has no native vision. Returns plain-text description. **Always use this first** if the user sends a photo and asks about it.
- **`generate-image "<prompt>" [src.jpg ...]`** — fal.ai FLUX.2 pro (text-to-image $0.03) or Kontext (edit with source image $0.04). Default for image generation/editing.
- **`generate-flux "<prompt>" [reference.jpg]`** — fal.ai FLUX.2 pro photorealistic ($0.03). Better for ultra-photoreal style transfer.
- **`edit-image bg-remove|upscale|segment-paint|inpaint|restyle|remove-object ...`** — fal.ai advanced edits ($0.00-0.055). Auto-mask + paint a zone, BG removal, upscale, etc.
- **`face-swap face.jpg target.jpg`** — fal.ai face swap to preserve a person's identity.
- **`generate-lora "trigger_word" "prompt"`** + **`train-lora "trigger" img1 img2 ...`** — Train and use FLUX LoRAs for consistent characters/styles.
...
Cuando generes un documento (PDF, imagen, audio, etc.) — sea con `generate-image`, `text-to-speech`, EasyBits, o cualquier tool — **siempre prefiere mandarlo como attachment vía `mcp__nanoclaw__send_file`** sobre pegar un URL público.

Razones:
- URLs públicos de servicios externos (EasyBits/Tigris, etc.) a veces vuelven 403/AccessDenied aunque el bucket diga "public".
- Attachment directo siempre llega al usuario.
- Sin links que expiren ni permisos que verificar.

Solo usa URL público cuando el archivo NO existe localmente (ej. el usuario explícitamente pide un link compartible para reenviar).
